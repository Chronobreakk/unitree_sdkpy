...
class Custom:
    ...
    def Init(self):
        ...
        # 定义基础姿态
        ready_pose = [0.06, 0.5, 0., 1.03, -1, 0.00, 0.00,    # 左臂放松姿态
                      0.06,-0.5, 0., 1.03, 1, 0.00, 0.00]     # 右臂放松姿态
        
        # point1_pose = [-0.060, 0.092, 0.218, 0.228, -1.299, 0.068, -1.061,
        #                -0.095, -0.368, -0.320, 0.275, 1.621, -0.015, 1.494]

        # point2_pose = [-0.095, 0.368, 0.320, 0.275, -1.621, 0.015, -1.494, 
        #                 -0.060, -0.092, -0.218, 0.228, 1.299, -0.068, 1.061]
        #wrist roll
        point1_pose = [0.02, 0.397, 0.251, 1.107, -1.748, 0.310, -1.723,
                       0.07, 0.009, -0.092, 0.810, 1.477, 0.221, 1.597]
# = - - = - - -
        point2_pose = [0.07, -0.009, 0.092, 0.810, -1.477, 0.096, -1.597, 
        ...
        self.action_sequence = [
            ...
            # 缓慢移动到准备位置
            {"time": 1, "poses": ready_pose},
        ]
        
        # 添加10次循环：点1 -> 点2 -> 点1
        cycle_duration = 2.0  # 每次移动4秒
        num_cycles = 20
        current_time = 0.5
        ...
        for cycle in range(num_cycles):
            ...
            self.action_sequence.append({"time": current_time, "poses": point1_pose})
            
            # 移动到点2
            current_time += cycle_duration
            self.action_sequence.append({"time": current_time, "poses": point2_pose})
        
        # 回到准备位置
        current_time += cycle_duration * 2  # 给更多时间回到准备位置
        self.action_sequence.append({"time": current_time, "poses": ready_pose})
